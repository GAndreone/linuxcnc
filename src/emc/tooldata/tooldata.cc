...
static int tooldata_mutex_get() {
    ...
    while ( rtapi_mutex_try(&the_mutex) ) { //true==failed
        ...
        fprintf(stderr,"!!!%8d UNEXPECTED: tooldata_mutex_get(): waited_us=%d\n"
               ,getpid(),waited_us);
        if (waited_us > maxwait_us) break;
    }
    if (waited_us > maxwait_us) {
        fprintf(stderr,"\n!!!%8d UNEXPECTED: tooldata_mutex_get(): FAIL\n",getpid());
...
struct CANON_TOOL_TABLE tooldata_get(int idx)
{
    ...
    if (idx > last_index ) {
       //print first 10 only
       if (idx < 10 ) {
           fprintf(stderr,"UNEXPECTED: tooldata_get() idx=%d>last_index=%d\n",
                  idx,last_index);
       }
    }
#endif //}
    ...
    if (tooldata_mutex_get()) {
        fprintf(stderr,"!!!%8d UNEXPECTED: tooldata_mutex_get() fail\n",getpid());
        fprintf(stderr,"!!!continuing without mutex\n");
    }
...
int tooldata_find_pocket(int tno)
{
    int pno;
    // start at 1 (skip spindle pocket)
    for (pno = 1; pno <= tooldata_last_index_get(); pno++) { //note <=
        CANON_TOOL_TABLE entry;
        entry = tooldata_get(pno);
        if (entry.toolno == tno) {
            return pno;
        }
    }
